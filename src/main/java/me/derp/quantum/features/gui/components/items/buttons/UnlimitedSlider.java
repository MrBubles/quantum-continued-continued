...
public class UnlimitedSlider
    ...
    public void drawScreen(int n, int n2, float f) {
        RenderUtil.drawRect(this.x, this.y, this.x + (float)this.width + 7.4f, this.y + (float)this.height - 0.5f, !this.isHovering(n, n2) ? Quantum.colorManager.getColorWithAlpha(Quantum.moduleManager.getModuleByClass(ClickGui.class).hoverAlpha.getValue()) : Quantum.colorManager.getColorWithAlpha(Quantum.moduleManager.getModuleByClass(ClickGui.class).alpha.getValue()));
        Quantum.textManager.drawStringWithShadow(" - " + this.setting.getName() + " " + ChatFormatting.GRAY + this.setting.getValue() + ChatFormatting.WHITE + " +", this.x + 2.3f, this.y - 1.7f - (float)OyVeyGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
    }
